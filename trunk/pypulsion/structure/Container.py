...
class Container(Data):
    ...
    def __init__(self,*args,**kwarg):
        ...
        self.__defaults__()
        if len(self):
            raise ValueError , 'Containers cannot have __defaults__'
    ...
    def check_new_val(self,val):
        ...
        if not isinstance(val,Data): 
            eption , 'val must be a Data() instance'        
        
        # make sure val has a tag
        if not val.has_key('tag'): 
            raise ValueError , 'val.tag must exist and be unique'
            #warn(Warning,'val.tag should exist')
            #val.tag = str(val.__class__).split('.')[-1].rstrip("'>")
        
        # make sure tag is unique
        ns = len(val.tag)
        if self.has_key(val.tag):
            #raise ValueError , 'val.tag=%s must exist and be unique'%val.tag
            warn('\nval.tag should be unique',Warning)
